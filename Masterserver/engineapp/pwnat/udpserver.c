...
/* internal functions */
static int handle_message(uint16_t id, uint8_t msg_type, char *data, int data_len, socket_t *from, list_t *clients, fd_set *client_fds, list_t *allowed_destinations, char *port);
static int destination_allowed(list_t *allowed_destinations, const char *host, const char *port);
...
int udpserver(int argc, char *argv[])
{
    ...
    {
        allowed_destinations = list_create(sizeof(destination_t),p_destination_cmp,p_destination_copy,p_destination_free);
    ...
    clients = list_create(sizeof(client_t), p_client_cmp, p_client_copy, p_client_free);
    if(!clients)
        goto done;
    ...
    udp_sock = sock_create((host_str[0] == 0 ? NULL : host_str), port_str,ipver, SOCK_TYPE_UDP, 1, 1);
    ...
    while(running)
    {
		...
		while ((ip = recv(listen_sock, packet, 100, 0)) > 0)
		{
			...
			*/
			//Windows XP Patch
			localIP = inet_ntoa (*(struct in_addr *)*host_ent->h_addr_list);
        ...
        if(timercmp(&curr_time, &check_time, >))
        {
            for(i = 0; i < LIST_LEN(clients); i++)
            {
                ...
                {
                    disconnect_and_remove_client(CLIENT_ID(client), clients, &client_fds);
                    i--;
                    continue;
                }
        ...
        {
            ret = msg_recv_msg(udp_sock, udp_from, data, sizeof(data),&tmp_id, &tmp_type, &tmp_len);
            
            if(ret == 0)
                ret = handle_message(tmp_id, tmp_type, data, tmp_len,udp_from, clients, &client_fds,allowed_destinations, port_str);
            if(ret == -2)
			{
        ...
        for(i = 0; i < LIST_LEN(clients) && num_fds > 0; i++)
        {
            ...
            if(client_tcp_fd_isset(client, &read_fds))
            {
                ...
                if(ret == 0)
					...
					_sleep(1);
	#else
					usleep(1000); /* Quick hack so doesn't use 100% CPU if data wasn't ready yet (waiting for ack) */
	#endif /*WIN32*/
#endif /*0*/
                if(ret == -2)
                {
